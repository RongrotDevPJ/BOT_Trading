...
class TradeExecutor:
    ...
    def get_positions(self, symbol=None):
        """Fetches active positions, optionally filtered by symbol."""
        if symbol:
            return ag.positions_get(symbol=symbol)
        return ag.positions_get()

    def open_position(self, side, lot, price, sl, tp, comment):
        """Helper to open a position based on side (BUY/SELL)."""
        order_type = ag.ORDER_TYPE_BUY if side == "BUY" else ag.ORDER_TYPE_SELL
        return self.send_order(config.SYMBOL, order_type, lot, price, sl, tp)
    ...
    def modify_position(self, ticket, sl, tp):
        ...
        request = {
            ...
            "position": ticket,
            "sl": self.normalize_price(sl, config.SYMBOL),
            "tp": self.normalize_price(tp, config.SYMBOL),
