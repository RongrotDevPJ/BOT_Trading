...
class SmartGridStrategy:
    ...
    def calculate_basket_tp(self, positions, side):
        ...
        tick_value = ag.symbol_info(config.SYMBOL).trade_tick_value
        tick_size = ag.symbol_info(config.SYMBOL).trade_tick_size
        
        if tick_value and tick_size:
            value_per_point = tick_value / (tick_size / point)
            target_points = config.BASKET_TP_DOLLARS / (total_volume * value_per_point)
        else:
            target_points = config.BASKET_TP_POINTS # Fallback

        # Calculate Basket TP
        if side == 0: # Buy basket
             basket_tp = break_even_price + (target_points * point)
        else: # Sell basket
             basket_tp = break_even_price - (target_points * point)
    ...
    def check_initial_entry(self, executor, current_rsi, tick):
        ...
        if current_rsi < config.RSI_BUY_LEVEL:
            self.logger.info(f"RSI Buy: RSI={current_rsi:.2f} < {config.RSI_BUY_LEVEL}")
            executor.send_order(config.SYMBOL, ag.ORDER_TYPE_BUY, config.START_LOT, tick.ask)
            
        # Only sell if RSI is overbought
        elif current_rsi > config.RSI_SELL_LEVEL:
            self.logger.info(f"RSI Sell: RSI={current_rsi:.2f} > {config.RSI_SELL_LEVEL}")
    ...
    def needs_new_grid_level(self, positions, current_price, side, current_atr):
        """
    ...
    def check_grid_logic(self, executor, current_atr):
        ...
        # Process Buy Grid
        if buy_positions:
            if len(buy_positions) < config.MAX_POSITIONS and self.needs_new_grid_level(buy_positions, current_ask, side=0, current_atr=current_atr):
        ...
        # Process Sell Grid
        if sell_positions:
             if len(sell_positions) < config.MAX_POSITIONS and self.needs_new_grid_level(sell_positions, current_bid, side=1, current_atr=current_atr):
